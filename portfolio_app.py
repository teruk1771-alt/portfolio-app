...
with st.expander("📉 買い増し検討チャート（損益率 -20%ライン）", expanded=True):
    # 同一銘柄をNISA・特定口座で保有している場合は取得総額加重平均で1本に集約
    _agg = df.groupby("銘柄", sort=False).agg(
        会社名=("会社名", "first"),
        取得総額=("取得総額", "sum"),
        評価額=("評価額", "sum"),
    ).reset_index()
    _agg["損益率"] = (_agg["評価額"] - _agg["取得総額"]) / _agg["取得総額"].clip(lower=1e-9)
    gain_df = _agg.copy()
    # 表示名：7文字で短縮（スマホ対応）
    def _short_name(r):
        name = r["会社名"] or r["銘柄"]
    ...
    # 損益率がほぼ0の銘柄でもバーが見えるよう表示用の値に最小幅を確保
    x_range = gain_df["損益率(%)"].abs().max() or 10
    MIN_BAR = x_range * 0.015   # 全体レンジの1.5%を最小幅とする
    gain_df["表示値"] = gain_df["損益率(%)"].apply(
        lambda v: -MIN_BAR if (-MIN_BAR < v <= 0) else (MIN_BAR if (0 < v < MIN_BAR) else v)
    )
    ...
    fig_gain.add_trace(go.Bar(
        ...
        text=[f"{v:.1f}%" for v in gain_df["損益率(%)"]],   # 実値を表示
        textposition="outside",       # 常にバー外に表示
        textfont=dict(size=11),
        hovertemplate=(
            "<b>%{y}</b><br>損益率: %{text}<extra></extra>"
        ),
    ))
    ...
    fig_gain.update_layout(
        title=dict(text="保有銘柄 損益率（取得価格比）", font=dict(size=13)),
        height=max(200, len(gain_df) * 36 + 60),
        xaxis=dict(
            title="",
            ticksuffix="%",
            ...
            zeroline=False,
            automargin=True,
            fixedrange=True,       # タッチズーム無効
            range=[-100, 150],  # 左端:-100% / 右端:150% 固定
        ),
        yaxis=dict(
            title="",
            tickfont=dict(size=10),
            fixedrange=True,       # タッチズーム無効
            side="right",          # 銘柄名を右端に配置
        ),
