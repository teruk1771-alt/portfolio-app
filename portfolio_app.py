...
with st.expander("📉 買い増し検討チャート（損益率 -20%ライン）", expanded=True):
    ...
    fig_gain.update_layout(
        ...
        height=max(180, len(gain_df) * 26 + 60),
        xaxis=dict(
            title="",
            ticksuffix="%",
            ...
            range=[
                min(gain_df["表示値"].min() * 1.3, -25),  # 左端：-20%ライン以上余白
                200,                                        # 右端：200%固定
            ],
        ),
        yaxis=dict(
        ...
        margin=dict(l=80, r=4, t=36, b=28),
        showlegend=False,
        plot_bgcolor="#fafafa",
        ...
        for i, (_, row) in enumerate(alerts.iterrows()):
            with cols[i % min(len(alerts), 3)]:
                st.markdown(
                    ...
                    f"{row['損益率']:.1%}</span><br>"
                    f"<small>{cur}{row['取得単価']:,.0f} → {cur}{row['現在株価']:,.0f}</small>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
    else:
